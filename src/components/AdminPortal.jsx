...
const AdminPortal = ({ userData, onLogout }) => {
  ...
  return <CivicShell userData={userData} onLogout={onLogout} menuItems={menuItems} searchPlaceholder="Search complaints">
    {(activeTab, activeItem) => activeTab === 'dashboard' ? <div className="civic-dashboard">
      ...
      <section className="civic-welcome"><div><p className="civic-eyebrow">Operations overview</p><h2>{getGreeting()}, {userData?.name || 'Administrator'}.</h2><p>See what needs attention and keep every complaint moving.</p></div><button>Open queue <ClipboardList size={17}/></button></section>
    ...
    </div> : <div className="civic-empty">{activeItem?.icon && React.createElement(activeItem.icon,{ size: 30 })}<h2>{activeItem?.label || 'Page'}</h2><p>This workspace is ready for your next feature.</p></div>}
  </CivicShell>;
};
